...
let set_bg ~attr ui =
  ...
  W.zbox
    ...
    ; ui
      |>$ Ui.size_sensor (fun ~w ~h ->
              if (w, h) <> Lwd.peek size then size $= (w, h) ) ]
...
let text_prompt ?pad_h ?pad_w ?(modify_body=fun x->x) ?(focus = Focus.make ()) ?(char_count = false)
    ~(show_prompt_var : text_prompt_data Option.t Lwd.var) ui =
  ...
  let prompt_args =
    ...
    |> Option.map
       ...
       Focus.request_reversable focus ;
       (*prefill the prompt if we want to *)
       if prompt_input |> Lwd.peek |> fst == "" then
         prompt_input $= (pre_fill, pre_fill |> String.length) ;
       let prompt_field =
         W.zbox
           [ W.string
               ~attr:A.(st underline)
       ...
       let label_bottom =
         ...
         if char_count then Some (prompt_val |> String.length |> Int.to_string)
         else None
       in
...
*)
let selection_list_prompt ?pad_w ?pad_h ?(modify_body=(fun x->x)) ?(focus = Focus.make ()) ~show_prompt_var ui =
  (*Build the ui so that it is either the prompt or nothing depending on whether show prompt is enabled*)
  let prompt_args =
    let$ show_prompt_val = Lwd.get show_prompt_var in
    show_prompt_val
    |> Option.map
       ...
       (*prefill the prompt if we want to *)
       let prompt_field =
         Selection_list.selection_list_custom ~focus
